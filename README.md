...
por coma (csv), el primero reporta el total de inicio de síntomas para casos confirmados por comuna en la semana epidemiológica, el segundo la serie de tiempo de los datos anteriores y el tercero indica las fechas que marcan los inicios y términos de cada semana semana epidemiológica. El primer archivo contiene los campos 'Región', 'Comuna', 'Población', '[semana epidemiológica]', donde la última columna tiene el 'Total de inicio de síntomas por caso confirmado' reportados en cada comuna, para cada semana epidemiológica. **Nota: estos datos son provisorios a la fecha del último reporte, pues se van actualizando retroactivamente a medida que se confirman casos**
